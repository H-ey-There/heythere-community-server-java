...
public class AmazonS3StorageService {
    ...
    public String upload(final MultipartFile file, final Post post, final String dirName) throws IOException {
        return upload(convert(file).get(), post, dirName);
    }

    public String upload(final File uploadFile, final Post post, final String dirName) {
        final String fileName = dirName + "/" + post.getId() + "-" + post.getTitle().trim() + "-" + uploadFile.getName();
